...
public class ObliqueProjection : MonoBehaviour {
	...
	public void LateUpdate () {
		...
		trackerPosition = transform.root.Find("ProjectionRoot").GetComponent<ProjectionRoot>().trackerPosition;
		stereo = transform.root.Find("ProjectionRoot").GetComponent<ProjectionRoot>().stereo;
		interpupillaryDistance = transform.root.Find("ProjectionRoot").GetComponent<ProjectionRoot>().interpupillaryDistance;
		
		if (stereo){
			if (whichEyeIfStereo == eyeOffset.left)
				trackerPosition.localPosition = new Vector3(-interpupillaryDistance/2, 0f, 0f);
			else
				trackerPosition.localPosition = new Vector3(interpupillaryDistance/2, 0f, 0f);
		}
		else
			trackerPosition.localPosition = Vector3.zero;
    	...
    	Matrix4x4 generatedProjection = KooimaPerspectiveProjection(
                ...
                TopLeftScreenCorner.localPosition, trackerPosition.localPosition, 
